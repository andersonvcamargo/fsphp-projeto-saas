...
function url_back(string $path): string
{
...
function theme(string $path,string $theme = CONF_VIEW_THEME): string
{
    ...
    if ($path) {
        return $base . "/themes/{$theme}/" . ltrim($path, "/");
    }
 ...
 * @return string
 */
function image(string $image, int $width, int $heigth): string
{
...
function request_limit(string $key, int $limit = 5, int $seconds = 60): bool
{
    ...
    if ($session->has($key)) {
        ...
        // Dentro do limite — incrementa e libera
        $session->set($key, [
            "time"     => $data->time,
    ...
    // Primeira requisição — inicializa
    $session->set($key, [
        "time"     => time() + $seconds,
        "requests" => 1
    ]);
