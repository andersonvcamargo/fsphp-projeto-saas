...
class Web extends Controller
{
    ...
    public function __construct()
    {
        ...
        $email->bootstrap(
            ...
            "Esté é apenas um teste de envio de email",
            "andersoncamargosi@gmail.com",
            "Anderson Camargo"
        )->sendQueue(); */
    }
    ...
    public function blog(?array $data): void
    {
        $head = $this->seo->render(
            "Blog -  " . CONF_SITE_NAME,
            "Confira em nosso blog dicas e sacadas de como controlar melhor sias contas. Vamos tomar um café?",
            url("/blog"),
            theme("/assets/images/share.jpg")
        );
        $blog = (new Post())->find();
    ...
    public function blogSearch(array $data): void
    {
        if (!empty($data['s'])) {
            ...
            $search = urlencode(
                htmlspecialchars(strip_tags($data['s']), ENT_QUOTES, 'UTF-8')
            );
        ...
        $search = htmlspecialchars(strip_tags($data['terms']));
        $page = htmlspecialchars(strip_tags($data['page'], FILTER_VALIDATE_INT) >= 1 ? $data['page'] : 1);
    ...
    public function register(?array $data): void
    {
        if (!empty($data['csrf'])) {
            ...
            if (!csrf_verify($data)) {
                echo json_encode([
                    "message" => $this->message->error("Erro ao enviar, favor use o formulario")->render()
                ]);
                return;
            }
            ...
            if (in_array("", $data)) {
                echo json_encode([
                    "message" => $this->message->info("Informe seus dados para criar sua conta.")->render()
                ]);
                return;
            }
    ...
    public function error(array $data): void
    {
        ...
        switch ($data['errcode']) {
            ...
            case "manutencao":
                ...
                $error->message = "Voltamos logo! Por hora estamos trabalhando para melhorar nosso contúdo para você controlar melhor suas contas :P";
            ...
            default:
                ...
                $error->message = "Sentimos muito, mas o contúdo que você tentou acessar não existe, está indisponivel no momento ou foi removido :/";
