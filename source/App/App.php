...
class App extends Controller
{
    ...
    public function home()
    {
        ...
        $income = (new AppInvoice())
            ->find(
                "user_id = :user AND type = 'income' AND status = 'unpaid' AND date(due_at) <= date(now() + INTERVAL 2 MONTH)",
                "user={$this->user->id}"
            )
            ->order("due_at")
            ->fetch(true);
        ...
        $expense = (new AppInvoice())
            ->find(
                "user_id = :user AND type = 'expense' AND status = 'unpaid' AND date(due_at) <= date(now() + INTERVAL 2 MONTH)",
                "user={$this->user->id}"
            )
            ->order("due_at")
            ->fetch(true);
    ...
    public function filter(array $data)
    {
        ...
        $date = (!empty($data["date"]) ? $data["date"] : "m/Y");
    ...
    public function invoice(array $data): void
    {
        ...
        if (!empty($data["update"])) {
            ...
            if (!$invoice) {
                $json["message"] = $this->message->error("Oooops! Não foi possivel carregar a fatura {$this->user->first_name}. Você pode tentar novamente.")->render();
                echo json_encode($json);
                return;
            }

            $dayofMonth = date("t", strtotime($invoice->due_at));
            if ($data["due_day"] < 1 || $data["due_day"] > $dayofMonth) {
        ...
        if (!$invoice) {
            $this->message->error("Oooops você tentou acessar uma fatura que não existe")->flash();
            redirect("/app");
        }
    ...
    public function remove(array $data): void
    {
        ...
        if (!$invoice || !$invoice->destroy()) {
            $this->message->error("Oooops! Não foi possível remover o lançamento. Tente novamente.")->flash();
